...
def latest_fvg_confluence_row(df: pd.DataFrame, idx: int, max_lookback=50):
    """Cherche le FVG le plus récent où le prix actuel est à l'intérieur"""
...
def place_market_order(symbol, side, sl, tp, info, df=None):
    ...
    if df is not None and len(df) >= 100:
        ...
        if current_spread > median_spread * 1.5:
            ...
            filling = mt5.ORDER_FILLING_FOK if info.filling_mode in (mt5.SYMBOL_FILLING_FOK, mt5.ORDER_FILLING_FOK) else mt5.ORDER_FILLING_IOC
    else:
        filling = mt5.ORDER_FILLING_FOK if info.filling_mode in (mt5.SYMBOL_FILLING_FOK, mt5.ORDER_FILLING_FOK) else mt5.ORDER_FILLING_IOC
...
def live_loop(symbol=SYMBOL_DEFAULT, timeframe="M1"):
    ...
    # Initialiser le modele ML avec l'historique complet
    ml = MLFilter()
    cooldown = 0
    last_bar_time = None
    ...
    while True:
        ...
        entry = df.at[i, 'close']
        if side=='buy':
            cands = df.iloc[max(0,i-60):i][df['swing_low']]
            ...
            cands = df.iloc[max(0,i-60):i][df['swing_high']]
    ...
    if args.mode == "backtest":
        ...
        if mt5.initialize(login=LOGIN, password=PASSWORD, server=SERVER):
            if mt5.symbol_select(symbol, True):
                ...
                ml_enabled = USE_ML_META_LABELLING and not args.no_plot
                ml_filter = MLFilter(
                ...
                if args.no_plot:
                    print(f"[ML] DESACTIVE en mode grid search")
                elif USE_ML_META_LABELLING:
